...
function getAnthropicKey() {
  try {
    ...
    console.error('No se pudo leer la clave Anthropic desde la DB:', e.message);
  }
...
function getClient() {
  ...
  if (!apiKey) {
    throw new Error('No hay clave de Claude AI configurada. Ve a Configuración → API y guarda tu clave.');
  }
...
router.post('/generate-product', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const prompt = `
Eres un experto en productos eléctricos e industriales para la empresa REElectrosistemas en Colombia.
...

{
  ...
  "description": "descripción detallada en HTML con párrafos y características técnicas",
...
router.post('/seo', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const prompt = `
Genera SEO optimizado en español para este producto eléctrico/industrial:
